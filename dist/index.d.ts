declare const VBase: {
    boolean: ((o: ModProxy<any>) => ModProxy<boolean> & SelfMap<DeMod<boolean, {
        optional: (o: ModProxy<any>) => OModProxy<any>;
    ...
    shape: (o: ModProxy<any>) => (<T extends {
        [K: string]: OModProxy<any> | RModProxy<any>;
    }>(shape: T) => ModProxy<DeOpt<T>> & SelfMap<DeMod<DeOpt<T>, {
        optional: (o: ModProxy<any>) => OModProxy<any>;
        custom: (o: ModProxy<any>) => (fn: (v: any) => boolean) => ModProxy<any>;
    } & {
...
declare type OModProxy<T> = ModProxy<T> & {
    _optional: true;
};
declare type RModProxy<T> = ModProxy<T> & {
    _optional?: never;
};
...
declare type DeMod<S, T extends ModSet<S>> = {
    [K in keyof T]: T[K] extends (o: ModProxy<any>) => OModProxy<S> ? OModProxy<S> : T[K] extends ((o: ModProxy<any>) => (...args: infer A) => ModProxy<S>) ? (...args: A) => OModProxy<S> : T[K] extends (o: ModProxy<any>) => ModProxy<S> ? ModProxy<S> : T[K] extends ((o: ModProxy<any>) => (...args: infer A) => ModProxy<S>) ? (...args: A) => ModProxy<S> : never;
};
declare type SelfMap<T> = {
    [K in keyof T]: T[K] extends ModProxy<any> ? T[K] & SelfMap<T> : T[K] extends (...args: infer A) => OModProxy<infer R> ? (...args: A) => OModProxy<R> & SelfMap<T> : T[K] extends (...args: infer A) => ModProxy<infer R> ? (...args: A) => ModProxy<R> & SelfMap<T> : never;
};
declare type Primitive = undefined | null | string | number | boolean | {};
declare type DeOpt<T extends {
    [K: string]: OModProxy<any> | RModProxy<any>;
}> = {
...
declare type OnlyOptKeys<T extends {
    [K: string]: OModProxy<any> | RModProxy<any>;
}> = {
    [K in keyof T]: T[K] extends OModProxy<any> ? K : never;
}[keyof T];
declare type OnlyReqKeys<T extends {
    [K: string]: OModProxy<any> | RModProxy<any>;
}> = {
